...
export default function Nba () {
    ...
    const fetchData = () =>{
        ...
        fetch('https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=24&q=nba&key=AIzaSyAWhMB1MsRJRjw4FkGU2OfZfSlW9YzcTHU',
        { method : 'GET',headers:new Headers({'Authorization': `Bearer ${accessToken}`})})     
        .then(res => res.json())
        .then(data => {
            setNbaVideo(data.items)
            setLoading(false)
            })
    }
    useEffect(()=>{
